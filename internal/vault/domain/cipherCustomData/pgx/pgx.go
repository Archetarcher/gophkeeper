...
// CipherCustomData is a database model
...
func (r *Repository) Get(ctx context.Context, login string) (*cipher.CipherCustomData, error) {
	var c CipherCustomData
	...
	return cipher.UnmarshalCipherCustomDataFromDatabase(c.Id, c.Key, c.Value, c.MetaData, c.UserId, c.CreatedAt, c.UpdatedAt, c.DeletedAt)
}
func (r *Repository) Add(ctx context.Context, u *cipher.CipherCustomData) error {
	_, err := r.db.NamedQueryContext(ctx, createQuery, CipherCustomData{
		Id:       u.GetId(),
		UserId:   u.GetUserId(),
		Key:      u.GetKey(),
		Value:    u.GetValue(),
...
func (r *Repository) Update(ctx context.Context, u *cipher.CipherCustomData) error {
